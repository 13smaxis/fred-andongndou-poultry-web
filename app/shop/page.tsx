...
export default function ShopPage() 
{
  ...
  useEffect(() => {
    ...
    {
      return;                                                                                                   //-If the carousel is paused or there is only one promotion, do not set up the auto-advance timer.
    }

    const timer = setInterval(() => {
      setActiveSlide((prev) => (prev + 1) % SHOP_PROMOTIONS.length);                                            //-Advance to the next slide every 4.5 seconds, looping back to the first slide after the last one.
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:py-10">
      ...
      >                                                                                                         {/* Promotions carousel section*/}
        <div className="relative h-56 sm:h-64 md:h-72">                                                         {/* Promotions carousel container*/}
          {SHOP_PROMOTIONS.map((slide, idx) => (
            ...
            >                                                                                                   {/* Carousel open div */}
              ...
              <div className="
                              absolute inset-0 
                              bg-linear-to-r 
                              from-green-900/80 via-green-900/55 to-transparent
                            " 
              />
          ...
          {SHOP_PROMOTIONS.map((slide, idx) => (
            <button
              ...
              className={`
                          h-2.5 rounded-full 
                          transition-all 
                          ${activeSlide === idx ? "w-8 bg-green-700" : "w-2.5 bg-green-200"}
                        `}
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">                                                     {/* Category filters */}
        <aside className="
                          rounded-2xl 
        ...
        <section>                                                                                               {/* Products section */}
          <div className="mb-4 flex items-center justify-between">
            ...
            <p className="text-sm text-gray-500">{filteredProducts.length} items</p>                          {/* Counts number of items filtered */}
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">                                {/* Structure of the product card */}
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="p-4">                                                                           {/* Card metadata */}
                  ...
                  <div className="mt-4 flex gap-2">                                                             {/* Cart and WhatsApp buttons */}
                    <button
                    ...
                    >
                      <MessageCircle className="h-4 w-4" />                                                     {/* WhatsApp icon size */}
                    </button>
                  </div>
                </div>
              </article>
